...
I.toArr(ElmBody.children, elm => {
    ELMENTS[I.Attr(elm, 'class').replace('emu-', '')] = elm;
});
M.aspect = 1.5;
M.PopupOptions = {};
...
I.assign(M.action, {
    async PlayNow() {
        ...
        var func = async e=> {
            ...
            if (coreName && coreJson[coreName]) {
                if (!M.romName) M.romName = urlinfo.get('game') || urlinfo.get('rom');
                if (!M.biosFile) M.biosFile = urlinfo.get('bios');
                if (!I.none(W.emu_rom)) M.romName = W.emu_rom;
                if (!I.none(W.emu_bios)) M.biosFile = W.emu_bios;
                M.coreName = coreName;
                M.coreInfo = coreJson[coreName];
                if (['true', true].includes(M.biosFile)) {
                    if (M.coreInfo['bios']) M.biosFile = M.coreInfo['bios']['files'];
                    else M.biosFile = null;
                } else if (M.biosFile) {
                    M.biosFile = I.str(M.biosFile) ? T.split(M.biosFile) : I.array(M.biosFile) ? M.biosFile : null;
                }
                M.CF('HtmlInstall');
            } else {
                M.CF('SelectCores');
            ...
            func = null;
            if(e&&I.elm(e.target))e.target.remove();
        };
        if (!M.JSpath.includes(location.host)) {
            T.once(T.$append(ELMENTS['welcome'],T.$ct('button',T.getLang('Play Now!'),"emu-game-start")), 'click', func);
        } else {
            func();
        }
    },
    ...
    HtmlSetting() {
        ...
        T.on(T.$append(homediv, T.$ct('button', `<span class="${formatClass('spopup-span','option')}">${T.getLang('Close Menu')}</span>`, formatClass('spopup-btn', 'key'))), 'pointerup', e => M.CF('ShowPopup', 'home', e, !0));
    ...
    HtmlDialog() {
        I.toArr({
                ...
                cheat(Elm, itemName) {
                    ...
                    T.on(Elm, 'keydown keyup keypress', e => T.stopProp(e), {
                        passive: true
                    });
                    T.on(Elm, 'opendialog', async function () {
                        this.hidden = false;
            ...
            entry => {
                ...
                Elm.hidden = true;
                Elm.innerHTML = `<div class="${formatClass('dialog-container',itemName)}"></div><div class="${formatClass('dialog-footer',itemName)}"><button class="${formatClass('dialog-btn','close',itemName)}">${T.getLang('Close Dialog')}</button></div>`;
    ...
    async HtmlWelcome() {
        var system = M.sysName,
            ...
            makeBtn = (gamename, text, key, type) => `<p class="emu-wel-p">${type||''}${gamename}</p><button class="emu-wel-btn" data-${key}="${gamename}">${T.getLang(text)}</button>`,
            creatBtn = (gamename, text) => makeBtn(gamename, text, 'path', ''),
            biosBtn = (gamename) => makeBtn(gamename, 'download', 'down', 'BIOS:'),
                ...
                resultfunc = async (e, index) => {
                    ...
                    if (data.path) {
                        ...
                        let name = F.getname(data.down),
                            ...
                            wpath = "";
                        if (index) {
                            url = ROOT.bios + data.down;
                            wpath = '/system/';
                            if (M.coreInfo['bios'] && M.coreInfo['path']) {
                                wpath += M.coreInfo['path'];
                            }
                        }
                        elm.parentNode.remove();
                        await T.FetchItem({
                            ...
                            async success(u8) {
                                let mime = F.CheckExt(u8);
                                if ((mime == 'zip' && M.coreInfo['extensions'].includes(mime)) || ['7z', 'rar'].includes(mime)) {
                                    let elm = T.$append(WelResult, T.$ce('div'));
                                    u8 = await F.unFile(u8, e => elm.innerHTML = name + ' ' + e);
                                }
                                if (I.u8buf(u8)) DISK.MKFILE(wpath + name, u8);
                                else I.toArr(v => DISK.MKFILE((index ? '/system/' + v[0] : F.gamename(v[0])), v[1]));
        ...
        I.toArr({
            ...
            if (entry[0] == 'bios' && M.coreInfo['bios']) {
                I.toArr(M.coreInfo['bios']['files'], b => {
                    if (entry[1].includes(sysExt + F.getname(b))) return;
                    html += `<li class="emu-wel-li">${biosBtn(b)}</li>`
                });
            }
            if (html) {
                elm.innerHTML = html;
    ...
    async RuntimeInitialized() {
        ...
        if (!M.FS.analyzePath(cfgpath).exists) {
            ...
            text += T.getLang('RetroArch Config Write:') + F.getname(cfgpath) + '\n';
        ...
        I.toArr(await T.FetchItem({
            ...
            entry => {
                ...
                text += T.getLang('FS Shader Write:') + entry[0] + '\n';
        ...
        I.toArr(await T.FetchItem({
            ...
            entry => {
                text += T.getLang('FS UI Write:') + entry[0] + '\n';
        ...
        if (M.romName) {
            return M.CF('WriteRoms');
        } else {
            T.$(resultID).innerHTML = `<div class="emu-wel-menu"></div><ul class="emu-wel-game"></ul><ul class="emu-wel-bios"></ul><ul class="emu-wel-roms"></ul><ul class="emu-wel-result"></ul>`;
    ...
    async WriteRoms() {
        if (!M.gameName || !M.FS.analyzePath(M.gameName).exists) {
            let system = M.sysName,
                sysExt = system + '-';
            if (M.romName) {
                let gameName = F.getname(M.romName),
                    u8 = await T.FetchItem({
                        url: M.romName,
                        key: sysExt + gameName,
                        store: T.LibStore,
                        dataOption: {
                ...
                let mime = F.CheckExt(u8);
                if ((mime == 'zip' && M.coreInfo['extensions'].includes(mime)) || ['7z', 'rar'].includes(mime)) {
                    u8 = await F.unFile(u8, e => console.log(gameName + ' ' + e));
                }
                if (I.u8buf(u8)) {
                    DISK.MKFILE(gameName, u8, 1);
                    M.gameName = gameName;
                } else I.toArr(x => {
                    let xname = F.getname(x[0]),
                        ext = F.getExt(xname);
                    if (!(/^\./.test(xname)) && M.coreInfo['extensions'].includes(ext)) {
                        DISK.MKFILE(xname, x[1], 1);
                        M.gameName = xname;
                    }
                });
            }
            if (I.array(M.biosFile)) {
                await I.Async(M.biosFile.map(async v => {
                    let name = F.getname(v),
                        url, wpath;
                    url = /^(http|blob)/.test(v) ? v : ROOT.bios + v;
                    wpath = '/system/';
                    if (M.coreInfo['bios'] && M.coreInfo['path']) {
                        wpath += M.coreInfo['path'];
                    }
                    await T.FetchItem({
                        ...
                        async success(u8) {
                            let mime = F.CheckExt(u8);
                            if ((mime == 'zip' && M.coreInfo['extensions'].includes(mime)) || ['7z', 'rar'].includes(mime)) {
                                u8 = await F.unFile(u8, e => console.log(name + ' ' + e));
                            }
                            if (I.u8buf(u8)) DISK.MKFILE(wpath + name, u8);
                            else I.toArr(x => DISK.MKFILE('/system/' + x[0], x[1]));
                            console.log(v + '  ' + T.getLang('is write'));
                        }
                    });
                }));
            }
            if (M.gameName) {
                M.arguments[1] = M.gameName;
                if (M.RA.context.state != "running") T.on(T.$append(ELMENTS['welcome'], T.$ct('button', T.getLang('Play Now'),'emu-game-start')), 'click', async e => {
    ...
    async callMain() {
        if (M.RA.context.state != "running") {
            ELMENTS['resume'].hidden = false;
            T.once(ELMENTS['resume'], 'click', async e => {
                //M.RA.context = new window["AudioContext"] || window["webkitAudioContext"];
                await M.RA.context.resume();
    ...
    BindEvent() {
            ...
            M.resizeTimer = setTimeout(() => {
                ...
                if (M.setCanvasSize) {
                    ...
                    if (M.keyMap) {
                        ...
                        setTimeout(() => M.CF('keyClick', M.keyMap['menu_toggle']), 100);
                    }
                }
            }, 300);
        });
        ...
        T.on(M.RA.context, 'statechange', e => {
            Module.CF('resume');
            console.log(e);
        });
        ...
        I.toArr(['touchstart', 'touchmove', 'touchend', 'touchcancel'], evt => {
            ...
            T.on(ELMENTS['controls'], evt, e => {
                ...
                if (e.type == 'touchstart') {
                    newlist = touchlist.concat(M.CF('dataKey', e.target));
                ...
                if (newlist.join(',') != touchlist) {
                    M.CF('keyChange', touchlist, newlist);
                    touchlist = newlist;
                }
                T.stopEvent(e);
            }, {
    ...
    keyClick(key) {
        M.CF('keyEvent', key, !0);
        setTimeout(() => M.CF('keyEvent', key, !1), 50);
    },
    dataKey(elm) {
...
class NengeDisk {
    ...
    mount(mount) {
        ...
        if (D.getStore(mount)) {
            ...
            D.__mount.push(D.syncfs(mount, txt => D.CF('DiskReadyOut', txt)));
        }
        return node;
    }
    mountReady() {
    ...
    async syncWrite(store, mount) {
        ...
        D.CF('indexdb-sync', IsReady, result);
        return result.join("\n");
    }
    loadLocalEntry(path) {
        let D = this,
